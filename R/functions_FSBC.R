#' Check self overlapping regions of the string. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Calculate the probability of string. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
fsbc_calc_prob_subseq <- function(x, symbols, ...) {
	res <- 1
	for(i in 1:nchar(x)) res <- res * symbols[stringr::str_sub(x, i, i)]
	res <- as.numeric(res)
	res
}

#' Calculation of probability of sequences including specific string. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Extend string with each symbol. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Get ratio of nucleobases. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Get permutations. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Get the set of initial strings. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' This is a wrapper function of "scale" to avoid errors of NULL. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
fsbc_scale <- function(x, ...) {
	...
	res <- c(scale(x, ...)) %>% set_names(.data, x)
	return(res)
}

#' Calculate Z-score, of string. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
...
#' @param ... do not use.
#'
#' @return results of subseq scores
#'
#' @import Biostrings
#' @import magrittr
#'
fsbc_calc_subseq_score <- function(x, y, symbols, freq, L = width(y), fixed = TRUE, ...) {

	PL <- lapply(x, fsbc_calc_prob_seq, symbols, L = max(L)) %>%
		do.call(cbind, .data) %>% set_names (.data, x)
...
#' Select strings with eliminating the search space. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Generate clusters based on string information. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
fsbc_seq_cluster <- function(x, y, res = list(), ...) {
	...
	r <- y[rows]
	res <- c(res, list(r)) %>% set_names(.data, c(names(res), x[1]))
...
#' Add cluster IDs to names of DNAStringSet dataset. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
...
#' Calculate frequency of sequence. FSBC functions are from "FSBC: FAST STRING-BASED CLUSTERING FOR HT-SELEX DATA" by Kato et al. (DOI: 10.1186/s12859-020-03607-1).
#'
